    ...
    for stock in stk_idx_lst:
        ...
        try:
            ...
            if len(stk_lst_data) == 0:
                Get_Price_History_Args = {'stock':stk_name,'ignore_date':True,'adjust_price':False,'show_weekday':False,'double_date':True}
                df = run_func(fpy.Get_Price_History, Get_Price_History_Args)
                df['Jdate'] = df.index
                ...
                if stk_lst_date != str(current_date):
                    ...
                    Get_Price_History_Args = {'stock':stk_name,'ignore_date':False,'adjust_price':False,'show_weekday':False,'double_date':True, 'start_date':start_date, 'end_date':end_date}
                    df = run_func(fpy.Get_Price_History, Get_Price_History_Args)
                    if len(df) > 0:
                        df['Jdate'] = df.index
                        query_utils.inserertDF(df)
